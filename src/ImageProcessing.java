...
public class ImageProcessing {
	...
	/*
	 * Forms the name of the output file by concatenating the name of the image
	 * input file with "_output_" and the names of the operations applied to the
	 * image file separated by underscores.
	 */
	private String createOutputImageName(String inputPath, String op) {
		// Partition the path to the input file into sub-strings separated by back
		// slashes
		String[] parts;
	...
	/*
	 * Process the input image file by applying to it all the operations specified
		...
		try {
			...
			try {
				...
				while ((line = br.readLine()) != null) {
					...
					// Split the line read from the configuration file into its components: color
					// mode,
					...
					if (loader.loadImage(parameters[1])) {
						...
						if ("Color".equals(parameters[0])) {
							...
							System.out.println(
									"Error: Please provide Color or Grayscale in configuration file: " + parameters[1]);
							br.close();
							return;
						}

						// This string will contain all image operations performed. This will be used as
						// part
				...
				if (filesProcessed == 1)
					...
					System.out.println(filesProcessed + " image files processed");
				System.exit(0);
